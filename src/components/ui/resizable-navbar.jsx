...
export const NavBody = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "relative z-[60] mx-auto hidden w-full max-w-7xl flex-row items-center justify-between self-start rounded-full bg-black px-4 py-3 lg:flex",
...
export const NavItems = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "absolute inset-0 hidden flex-1 flex-row items-center justify-center space-x-2  text-[15px] font-medium text-zinc-600 transition duration-200 hover:text-zinc-800 lg:flex lg:space-x-2",
        className
      )}>
      {items.map((item, idx) => (
        <Link
          ...
          onClick={onItemClick}
          className="relative px-2 py-1 text-neutral-300 hover:text-black"
          key={`link-${idx}`}
          href={item.link}>
          {hovered === idx && (
            <motion.div
...
export const MobileNavHeader = ({
  ...
  return (
    <div
      className={cn("flex w-full flex-row items-center justify-between px-3", className)}>
      {children}
    </div>
  );
};
